...
//Rare exceptions to indicate a bug in the _smith_carl_project2 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_smith_carl_project2 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_smith_carl_project2]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.smith_carl_project2.module"
