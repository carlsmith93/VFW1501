...
NSString * Smith_Carl_Project4$ModuleRequireFormat = @"(function(exports){"
...
@implementation Smith_Carl_Project4Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_smith_carl_project4);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Smith_Carl_Project4 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Smith_Carl_Project4" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_smith_carl_project4 = [[Smith_Carl_Project4Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_smith_carl_project4];
	
	NSString *_smith_carl_project4NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _smith_carl_project4NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_smith_carl_project4 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Smith_Carl_Project4$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.smith_carl_project4.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.smith_carl_project4.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
